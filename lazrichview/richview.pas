...
uses
  ...
  RVStyle, RVScroll, ClipBrd, Contnrs,
  {$IFDEF RICHVIEWDEF4}
  ImgList,
  {$ENDIF}
  ExtCtrls;
...
type
  TRichView = class;
  TRVSaveFormat = (rvsfText, rvsfHTML);
  TRVSaveOption = (rvsoOverrideImages);
  TRVAlign = (rvalLeft, rvalCenter, rvalRight);
  ...
  private
    ...
    fPrintLineNo: Boolean;
    fLineNoWidth: Integer;
    function GetVSmallStep: Integer;
    procedure InvalidateJumpRect(no: Integer);
    procedure SetPrintLineNo(AValue: Boolean);
  ...
  protected
    ...
    function JumpCheck(X, Y: Integer): Boolean;
    procedure DblClick; override;
    procedure KeyDown(var Key: Word; Shift: TShiftState); override;
    procedure FormatLine(no: Integer; var x, baseline: Integer;
      var prevdesc, prevabove:Integer; Canvas: TCanvas);
    procedure AdjustJumpsCoords;
    procedure AdjustChildrenCoords;
    procedure UpdateChildren; override;
  ...
  public
    ...
    procedure AddFromNewLine(s: String;StyleNo:Integer;Align:TRVAlign = rvalLeft);
    procedure Add(s: String;StyleNo:Integer);
    procedure AddCenterLine(s: String;StyleNo:Integer);
    procedure AddText(s: String;StyleNo:Integer;Align:TRVAlign = rvalLeft);
    procedure AddTextFromNewLine(s: String;StyleNo:Integer;Align:TRVAlign = rvalLeft);
    procedure AddBreak;
    ...
    procedure AddControl(ctrl: TControl; Align:TRVAlign = rvalLeft);
    ...
    property VSmallStep: Integer read GetVSmallStep write SetVSmallStep;
    ...
    function Search(Keyword: String; Rewind: Boolean = False): Boolean;

    procedure SelectOnControl(Control: TControl);
    procedure ScrollOnControl(Control: TControl; Delta: Integer);
    procedure MouseUpOnControl(Control: TControl; Button: TMouseButton;
      X, Y: Integer);
  ...
  published
    ...
    property PrintLineNo: Boolean read fPrintLineNo write SetPrintLineNo;
  end;

implementation

uses ChatLabel, strutils;
...
function TRichView.Search(Keyword: String; Rewind: Boolean): Boolean;
var
  StartNo, StartOffs, EndOffs, LineNo: Integer;
  Line: String;
  i, a,b,mid, midtop: Integer;
  dli: TDrawLineInfo;
begin
  Result := False;
  if Length(Keyword) < 1 then exit;
  if DrawLines.Count < 1 then exit;
  ...
  else if LowerCase(GetSelText) = LowerCase(Keyword) then
  begin
    if (fSelStartNo > fSelEndNo) or
      ((fSelStartNo = fSelEndNo) and (fSelEndOffs > fSelStartOffs)) then
    begin
      StartNo := fSelEndNo;
  ...
  begin
    Line := Lines[LineNo];
    StartOffs := PosEx(LowerCase(Keyword), LowerCase(Line), StartOffs);
    if StartOffs > 0 then
    begin
      EndOffs := StartOffs + Length(Keyword);
      RestoreSelBounds(LineNo, LineNo, StartOffs, EndOffs);
      break;
    end;
    inc(LineNo);
    StartOffs := 1;
  ...
  if fSelStartNo <> -1 then
  begin
    dli := TDrawLineInfo(DrawLines.Objects[fSelStartNo]);
    if (dli.Top + dli.Height) > (VPos * SmallStep + ClientHeight) then
      ScrollTo(dli.Top + dli.Height - ClientHeight)
    else if dli.Top < (VPos * SmallStep) then
      ScrollTo(dli.Top);
    Refresh;
    Result := True;
  end;
...
{-------------------------------------}
procedure TRichView.SelectOnControl(Control: TControl);
begin
  if (Control.Tag < 0) or (Control.Tag >= DrawLines.Count) then exit;
  fSelStartNo := TDrawLineInfo(DrawLines.Objects[Control.Tag]).LineNo;
  fSelStartOffs := 0;
  fSelEndNo := TDrawLineInfo(DrawLines.Objects[Control.Tag]).LineNo;
  fSelEndOffs := 0;
  ScrollDelta := 0;
...
procedure TRichView.ScrollOnControl(Control: TControl; Delta: Integer);
begin
  ScrollDelta := Delta;
end;

procedure TRichView.MouseUpOnControl(Control: TControl; Button: TMouseButton;
  X, Y: Integer);
begin
  if ScrollTimer<> nil then begin
    ScrollTimer.Free;
  ...
  if (Control.Tag = fSelStartNo) and (fSelStartNo = fSelEndNo) then
  begin
    Invalidate;
    if Assigned(FOnSelect) then FOnSelect(Self);
  end else if Selection and (Button = mbLeft) then begin
    Selection := False;
    Invalidate;
    if Assigned(FOnSelect) then FOnSelect(Self);
  end;
...
{-------------------------------------}
procedure TRichView.AddFromNewLine(s: String; StyleNo: Integer; Align: TRVAlign
  );
var info: TLineInfo;
begin
...
{-------------------------------------}
procedure TRichView.AddText(s: String; StyleNo: Integer; Align: TRVAlign);
var p: Integer;
begin
   {$IFDEF FPC}
   ...
   while s<>'' do begin
     p := Pos(chr(13)+chr(10),s);
     if p=0 then begin
        AddFromNewLine(s,StyleNo,Align);
        break;
     end;
     AddFromNewLine(Copy(s,1,p-1), StyleNo,Align);
     Delete(s,1, p+1);
   end;
end;
{-------------------------------------}
...
{-------------------------------------}
procedure TRichView.AddControl(ctrl: TControl; Align: TRVAlign); { do not free ctrl! }
var info: TLineInfo;
begin
  info := TLineInfo.Create;
  info.StyleNo := -5;
...
begin
  ...
  if (csDesigning in ComponentState) or
    not Assigned(FStyle) or skipformatting or (depth>1) then
    exit;
  skipformatting := True;
  ...
  begin
    ...
    if fPrintLineNo then
    begin
      Canvas.Font.Name := '';
      Canvas.Font.Size := 9;
      Canvas.Font.Style := [];
      Canvas.Font.Color := clWindowText;
      fLineNoWidth := Canvas.TextWidth(IntToStr(Lines.Count)) + 5;
    end else
      fLineNoWidth := 0;
  end;

  OldY := VPos * SmallStep;
  ScrollLock := (OldY + ClientHeight + 10) < TextHeight;

  oldtextwidth := TextWidth;

  mx := max(ClientWidth-(fLineNoWidth+FLeftMargin+FRightMargin), GetMaxPictureWidth);
  if mx<FMinTextWidth then mx := FMinTextWidth;
  if FClientTextWidth then begin { widths of pictures and maxtextwidth are ignored }
    TextWidth := ClientWidth-(fLineNoWidth+FLeftMargin+FRightMargin);
  ...
  for i:=StartLine to lines.Count-1 do begin
    ...
    if not (((StyleNo = rvsPicture) and
      (not (rvdoImages in DisplayOptions))) or
      ((StyleNo = rvsComponent) and
      (not (rvdoComponents in DisplayOptions))) or
      (((StyleNo = rvsBullet) or (StyleNo = rvsHotspot)) and
      (not (rvdoBullets in DisplayOptions)))) then
      FormatLine(i,x,b, d,a, Canvas);
  end;
  TextHeight := b+d+1;
  ...
  cw := ClientWidth;
  ch := ClientHeight;
  UpdateScrollBars(mx + fLineNoWidth + FLeftMargin + FRightMargin, TextHeight);
...
begin
  for i:=0 to drawlines.Count-1 do begin
    ...
    begin
      ...
      TControl(li.gr).Top := dli.Top;
      TControl(li.gr).Tag := i;
    end;
  end;
end;
{-------------------------------------}
procedure TRichView.UpdateChildren;
var
  i: Integer;
  dli: TDrawLineInfo;
  li : TLineInfo;
begin
  ...
  begin
    ...
    dli := TDrawLineInfo(drawlines.Objects[Controls[i].Tag]);
    li := TLineInfo(lines.Objects[dli.LineNo]);
    Controls[i].Top := dli.Top - VPos*SmallStep;
    Controls[i].Left := dli.Left - HPos;
    Controls[i].Visible := ((Controls[i].Top + Controls[i].Height) >= 0) and
      (Controls[i].Top < ClientHeight) and
      ((Controls[i].Left + Controls[i].Width) >= 0) and
      (Controls[i].Left < ClientWidth);
  end;
  inherited UpdateChildren;
end;
{-------------------------------------}
procedure TRichView.FormatLine(no: Integer; var x, baseline: Integer;
  var prevdesc, prevabove: Integer; Canvas: TCanvas);
var
...
begin
  width := TextWidth;
  startline := drawlines.Count-1;
  while (startline >= 0) do
  begin
    if TDrawLineInfo(drawlines.Objects[startline]).FromNewLine then break;
    dec(startline);
  end;
  if (drawlines.Count > 0) then
    Align := TDrawLineInfo(drawlines.Objects[startline]).Align
  else
    Align := rvalLeft;
  ...
  case TLineInfo(lines.Objects[no]).StyleNo of
    -5: { Control }
    begin
      if TLineInfo(lines.Objects[no]).gr is TChatLabel then
        (TLineInfo(lines.Objects[no]).gr as TChatLabel).ForceCalculateSize;
      ...
      info.Align  := TLineInfo(lines.Objects[no]).Align;
      info.LineNo := no;
      info.Top    := baseline + prevdesc + 1;
      info.Width  := ctrlw;
      info.Height := ctrlh+1;
      if info.Align = rvalCenter then
      begin
        info.Left := (width-ctrlw) div 2;
        if info.Left<0 then info.Left := 0;
        inc(info.Left, fLineNoWidth + fLeftMargin);
      end
      else if info.Align = rvalRight then
      begin
        info.Left := (width-ctrlw);
        inc(info.Left, fLineNoWidth + fLeftMargin);
      end
      else
        info.Left := fLineNoWidth + fLeftMargin;
      drawlines.AddObject('',info);
      TControl(TLineInfo(lines.Objects[no]).gr).Tag := drawlines.Count - 1;
    ...
    begin
      ...
      info.Width  := ctrlw+1;
      info.Height := ctrlh+1;
      if TLineInfo(lines.Objects[no]).SameAsPrev and (Align = rvalRight) then
      begin
        if TDrawLineInfo(drawlines.Objects[startline]).Left < (ctrlw+2) then
        begin
        end
        else
        begin
          for j := drawlines.Count-1 downto startline do
            dec(TDrawLineInfo(drawlines.Objects[j]).Left, ctrlw+2);
          dec(x, ctrlw+2);
        end;
      end;
      if not TLineInfo(lines.Objects[no]).SameAsPrev or (x+ctrlw+2 > width) then
      begin
        x:=0;
        y:=baseline + prevdesc;
        inc (baseline,prevdesc+ctrlh+1);
      ...
      if TLineInfo(lines.Objects[no]).StyleNo = -4 then begin{HotSpot}
        ...
        jmpinfo.l   := x + 1 + fLineNoWidth + fLeftMargin;;
        jmpinfo.t   := y + 1;
        jmpinfo.w   := ctrlw;
        jmpinfo.h   := ctrlh;
        jmpinfo.id  := nJmps;
      ...
      info.Left := x + 1 + fLineNoWidth + fLeftMargin;;
      inc(x, ctrlw+2);
      info.Top :=  y+1;
      info.LineNo := no;
    ...
    begin
      ...
      Align := TLineInfo(lines.Objects[no]).Align;
      if (sourceStrPtrLen = 0) and newline then begin
        info := TDrawLineInfo.Create;
        info.Align := rvalLeft;
        info.LineNo := no;
        info.Offs := 0;
        ...
        info.Left   := fLineNoWidth + fLeftMargin;;
        info.Top    := baseline+prevDesc+metr.tmExternalLeading;
        info.Width  := 0;
        info.Height := 0;
        drawlines.AddObject('',info);
        inc(baseline, prevDesc+metr.tmExternalLeading+metr.tmAscent);
        prevabove := metr.tmExternalLeading+metr.tmAscent;
        if newline or (prevDesc < metr.tmDescent) then
          prevDesc := metr.tmDescent;
      end;
      while sourceStrPtrLen>0 do begin
        ...
        begin {continue line}
          ...
          begin
            ...
            if j>=0 then
            repeat
              inc(TDrawLineInfo(drawlines.Objects[j]).Top,
        ...
        begin { new line }
          info.FromNewLine := True;
          info.Align := Align;
          if Align = rvalLeft then
            x := 0
          else if Align = rvalCenter then
            x := (Width - sz.cx) div 2
          else
            x := Width - sz.cx;
        ...
        info.Left   := x+fLineNoWidth+fLeftMargin;;
        info.Top    := y;
        info.Width  := sz.cx;
        info.Height := sz.cy;
        ...
        begin
          jmpinfo := TJumpInfo.Create;
          jmpinfo.l := x+fLineNoWidth+fLeftMargin;
          jmpinfo.t := y;
          jmpinfo.w := sz.cx;
          jmpinfo.h := sz.cy;
          jmpinfo.id := nJmps;
...
begin
  ...
  begin
    ...
    if no>=0 then
    begin { text }
      if fPrintLineNo and (dli.Offs <= 1) then
      begin
        canv.Font.Name := '';
        canv.Font.Size := 9;
        canv.Font.Style := [];
        canv.Font.Color := clLtGray;
        LineNo := IntToStr(dli.LineNo + 1);
        LineNoWidth := canv.TextWidth(LineNo);
        canv.TextOut(fLeftMargin + fLineNoWidth - LineNoWidth - 5 - xshift,
          dli.Top - yshift, IntToStr(dli.LineNo + 1));
      end;
      ...
      begin
        ...
        canv.Brush.Color := FStyle.SelColor;
        if not hovernow then canv.Font.Color := FStyle.SelTextColor;
        if (i<>EndNo) or (EndOffs>Length(DrawLines[i])) then
        begin
          {$IFDEF FPC}
    ...
    begin { hotspots and bullets }
      ...
      begin
        ...
        TImageList(li.gr).DrawingStyle := TDrawingStyle.dsSelected;
      end;
    ...
    { controls ignored }
    if (no = -5) and (TControl(li.gr) is TChatLabel) then
    begin
      if (StartNo<=i) and (EndNo>=i) and (StartNo <> EndNo) then
        TChatLabel(li.gr).SelectAll
      else if (StartNo <> EndNo) or (i <> StartNo) then
        TChatLabel(li.gr).Deselect;
    end;
  end;
  ...
  begin
    ...
    canv.Line(fLeftMargin + fLineNoWidth - 3 - xshift, 0,
      fLeftMargin + fLineNoWidth - 3 - xshift, buffer.Height - 1);
  end;
...
procedure TRichView.SetPrintLineNo(AValue: Boolean);
begin
  if fPrintLineNo=AValue then Exit;
  fPrintLineNo:=AValue;
  Format;
  Refresh;
end;

function TRichView.GetVSmallStep: Integer;
begin
  Result := SmallStep;
end;
...
var i, no, offs,ys, oldjump: Integer;
begin
  ...
  else if ScrollControl = nil then
  begin
    oldjump := LastJumpMovedAbove;
    if JumpCheck(X, Y) then
    begin
      if Assigned(FOnRVMouseMove) and
         (LastJumpMovedAbove <> oldjump) then
        OnRVMouseMove(Self,LastJumpMovedAbove+FirstJumpNo);
      exit;
    end;
    if DrawHover and (LastJumpMovedAbove<>-1) then begin
      DrawHover := False;
...
begin
    ...
    XClicked := X;
    YClicked := Y;
    if (fSelStartNo >= 0) and (fSelStartNo = fSelEndNo) and
      (fSelStartOffs = 0) and (fSelEndOffs = 0) and not Selection and
      (TLineInfo(Lines.Objects[fSelStartNo]).StyleNo = -5) and
      ((TLineInfo(Lines.Objects[fSelStartNo]).gr as TControl) is TChatLabel) then
    begin
      (TLineInfo(Lines.Objects[fSelStartNo]).gr as TChatLabel).MouseUpOnParent
        (Button, X, Y);
      if Assigned(FOnSelect) then FOnSelect(Self);
      exit;
    end;
...
begin
  ...
  //if Assigned(FOnJump) then begin
    ...
    if {LastJumpDowned=-1} AllowSelection then begin
      for i := 0 to ControlCount - 1 do
      begin
        if Controls[i] is TChatLabel then
          (Controls[i] as TChatLabel).Deselect;
      end;
...
function TRichView.JumpCheck(X, Y: Integer): Boolean;
var
  i: Integer;
begin
  Result := True;
...
begin
  inherited DblClick;
  if SingleClick then exit;
  if Assigned(FOnRVDblClick) then
  begin
    if FindClickedWord(clickedword, StyleNo) then
       FOnRVDblClick(Self, clickedword, StyleNo);
    exit;
  ...
  begin
    ...
    begin
      ...
      if max>Length(clickedword) then max := Length(clickedword);
      first := max;
      if (Pos(clickedword[first], Delimiters) = 0) then
      begin
        while (first>1) and (Pos(clickedword[first-1], Delimiters)=0) do
          dec(first);
        len := max-first+1;
        while (first+len-1<Length(clickedword)) and
          (Pos(clickedword[first+len], Delimiters)=0) do
          inc(len);
        clickedword := copy(clickedword, first, len);
...
begin
  GetSelBounds(StartNo, EndNo, StartOffs, EndOffs);
  if (StartNo = -1) or (EndNo = -1) then
    Result := False
  else if (StartNo=EndNo) and (StartOffs=EndOffs) then
  begin
    li := TLineInfo(Lines.Objects[StartNo]);
    if (li.StyleNo = -5) and (TControl(li.gr) is TChatLabel) then
      Result := True
    else
      Result := False;
  end
...
begin
  ...
  if StartNo = EndNo then begin
    ...
    begin
      if TControl(li.gr) is TChatLabel then
      begin
        Result := TChatLabel(li.gr).SelectText;
      end;
      exit;
    end;
    ...
    li := TLineInfo(Lines.Objects[StartNo]);
    if (li.StyleNo = -5) and (TControl(li.gr) is TChatLabel) then
      s := TChatLabel(li.gr).SelectText
    else if li.StyleNo < 0 then
      s := ''
    else
    ...
    for i := StartNo+1 to EndNo do begin
      ...
      begin
        TChatLabel(li.gr).SelectAll;
        s := s + TChatLabel(li.gr).SelectText;
      end
      else if li.StyleNo >= 0 then
        if i<>EndNo then
          s := s + Lines[i]
...
begin
  if ScrollDelta<>0 then begin
    if ScrollControl <> nil then
    begin
      if ((ScrollDelta < 0) and (ScrollControl.Top >= 0)) or
        ((ScrollDelta > 0) and
        ((ClientHeight - ScrollControl.Top) >= ScrollControl.Height)) then
        ScrollDelta := 0;

      if ScrollControl is TChatLabel then
        (ScrollControl as TChatLabel).ScrollDeltaOnParent(ScrollDelta);
    end;
