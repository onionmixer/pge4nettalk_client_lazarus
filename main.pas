...
begin
  if fUploadID = 0 then
  begin
    if fNetworkStatus = OZFTwistedKnotStatusDisconnect then
...
var
  ...
  cont: Boolean;
begin
  cargo := TOZFCargoCompany.Create(Receiver.Buffer, Receiver.Length);
...
begin
  ...
  if fNetworkStatus = OZFTwistedKnotStatusConnect then
  begin
    if fAuth then
       FormLogin.Retry;
  end;
